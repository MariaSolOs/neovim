...
M.get = memoize('concat-2', function(lang, query_name)
  ...
  if explicit_queries[lang][query_name] then
    ...
    for _, base_lang in ipairs(base_langs) do
      local base_files = M.get_files(base_lang, query_name, true)
      vim.list_extend(explicit_query_files, base_files)
    end
  end

  local query_files = M.get_files(lang, query_name)
  local query_string = read_query_files(explicit_query_files)
    .. (explicit_queries[lang][query_name] or '')
    .. read_query_files(query_files)
